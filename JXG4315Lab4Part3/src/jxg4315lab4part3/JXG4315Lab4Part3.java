...
public class JXG4315Lab4Part3 {
    ...
    public static void main(String[] args) {
        ...
        double[][] robotDbls = new double[MAXSIZE][4];
        final int HEIGHT = 0;
        final int WEIGHT = 1;
        final int TCOST = 2;
        
        double[] robotDecCost = new double[MAXSIZE];
        char[] robotDecQuant = new char[MAXSIZE];
        ...
        try 
        {
            ...
            outFile = new PrintWriter("JXG4315Lab4Part3.out");
            System.out.println("Output data file is JXG4315Lab4Part3.out");
        }
        ...
        while (inputFile.hasNext())
        {
            ...
            if (inputValid)
            {
                ...
                {
                    int count = 0;
                    for ( int i = 0; i < robotStrs[ri][COST].length(); i++)
                    {
                            count++;
                    }
                    robotDecQuant[ri] = robotStrs[ri][COST].charAt(count - 1);
                    robotDecCost[ri] = Double.parseDouble(robotStrs[ri][COST].replace("K", ""));
                ...
                if (robotStrs[ri][COST].contains("M"))
                {
                    ...
                    robotDecQuant[ri] = robotStrs[ri][COST].charAt(count - 1);
                    robotDecCost[ri] = Double.parseDouble(robotStrs[ri][COST].replace("M", ""));
                    
                    robotDbls[ri][TCOST] = Double.parseDouble(robotStrs[ri][COST].replace("M", ""));
                    robotDbls[ri][TCOST] = robotDbls[ri][TCOST] * 1000000;
                }
                outFile.println("robotDecQuant["+ri+"] = " + robotDecQuant[ri]);
                outFile.println("robotDecCost["+ri+"] = " + robotDecCost[ri]);
        ...
        bubbleSort(robotStrs, robotInts, robotDbls, robotChars, NAME, robotDecQuant, robotDecCost, outFile);
        printRobotTable(robotStrs, robotInts, robotDbls, robotChars, outFile);
        printRobotTable(robotStrs, robotInts, robotDbls, robotChars, robotDecQuant, robotDecCost, outFile);
        
        outFile.close();
    }
    
    public static void bubbleSort(String[][] names, int[][] vals, double[][] doubles, char[][] types, int sortBy, char[] decQuant, double[] decCost, PrintWriter outFile) 
    {
            ...
            for ( int inner = 0; inner < vals.length-1; inner++ )
            {
                if ((names[inner][sortBy] != null) && (names[inner + 1][sortBy] != null))
                    if (names[inner][sortBy].compareTo(names[inner + 1][sortBy]) > 0)
                    {
                        ...
                        tempCostValue = decCost[inner];
                        decCost[inner] = decCost[inner + 1];
                        decCost[inner + 1] = tempCostValue;

                        tempQuant = decQuant[inner];
                        decQuant[inner] = decQuant[inner + 1];
                        decQuant[inner + 1] = tempQuant;
                    }
    ...
    public static void printRobotTable(String[][] robotStrs, int[][] robotInts, double[][] robotDbls, char[][] robotChars, char[] robotDecQuant, double[] robotDecCost, PrintWriter outFile) {
        final int NAME = 0;
        final int SOURCE = 0;
        final int YEAR = 0;
        final int KIND = 1;
        ...
        for ( int i = 0; i < robotInts.length; i++ )
        {
            if (robotInts[i][YEAR] != 0)
            {
                System.out.printf("%15s%7d%10s%1s%3c%15s%10s%20s%22s  %-40s\n", robotStrs[i][NAME], robotInts[i][YEAR], robotDecCost[i], robotDecQuant[i], robotChars[i][SOURCE], robotDbls[i][HEIGHT], robotDbls[i][WEIGHT], robotStrs[i][ACTOR], robotStrs[i][KIND], robotStrs[i][TITLE]);
            }
        }
        ...
        for ( int i = 0; i < robotInts.length; i++ )
        {
            if (robotInts[i][YEAR] != 0)
            {
                outFile.printf("%15s%7d%10s%1s%3c%15s%10s%20s%22s  %-40s\n", robotStrs[i][NAME], robotInts[i][YEAR], robotDecCost[i], robotDecQuant[i], robotChars[i][SOURCE], robotDbls[i][HEIGHT], robotDbls[i][WEIGHT], robotStrs[i][ACTOR], robotStrs[i][KIND], robotStrs[i][TITLE]);
            }
        }
    }
