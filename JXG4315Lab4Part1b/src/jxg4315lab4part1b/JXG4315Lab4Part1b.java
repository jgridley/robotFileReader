 ...
 * @author jxg4315
 */
public class JXG4315Lab4Part1b {
    ...
    public static void main(String[] args) {
        ...
        try 
        {
            ...
            outFile = new PrintWriter("JXG4315Lab4Part1b.out");
            System.out.println("Output data file is JXG4315Lab4Part1b.out");
        }
        ...
        while (inputFile.hasNext())
        {
            ...
            if (inputValid)
            {
                ...
                outFile.println("Kind = robotStrs["+ri+"]["+KIND+"] = " + robotStrs[ri][KIND]);
                ...
                outFile.println("Cost = robotStrs["+ri+"]["+COST+"] = " + robotStrs[ri][COST]);
        ...
        for ( int i = 0; i < ri; i++ )
        {
            ...
            System.out.println(robotStrs[i][KIND] + " robot " + (i+1) + ": " + robotStrs[i][NAME] + " debuted " + robotInts[i][YEAR] + 
                    " in " + (robotChars[i][SOURCE] == 'R'? "real life from " + robotStrs[i][TITLE]
                            + " at the cost of $" + robotStrs[i][COST]:
    ...
    public static void printRobotTable(String[][] robotStrs, int[][] robotInts, double[][] robotDbls, char[][] robotChars, PrintWriter outFile) {
        ...
        final int COST = 4;
        System.out.println("\nTable of Robot Info");
        System.out.printf("%15s%7s%11s%8s%17s%7s\n", "NAME", "YEAR", "COST", "SOURCE", "CATEGORY", "TITLE");
        for ( int i = 0; i < robotInts.length; i++ )
        {
            if (robotInts[i][YEAR] != 0)
            {
                System.out.printf("%15s%7d%11s%3c%22s  %-40s\n", robotStrs[i][NAME], robotInts[i][YEAR], robotStrs[i][COST], robotChars[i][SOURCE], robotStrs[i][KIND], robotStrs[i][TITLE]);
            }
        }
        
        outFile.println("\nTable of Robot Info");
        outFile.printf("%15s%7s%11s%8s%17s%7s\n", "NAME", "YEAR", "COST", "SOURCE", "CATEGORY", "TITLE");
        for ( int i = 0; i < robotInts.length; i++ )
        {
            if (robotInts[i][YEAR] != 0)
            {
                outFile.printf("%15s%7d%11s%3c%22s  %-40s\n", robotStrs[i][NAME], robotInts[i][YEAR], robotStrs[i][COST], robotChars[i][SOURCE], robotStrs[i][KIND], robotStrs[i][TITLE]);
            }
        }
    }
